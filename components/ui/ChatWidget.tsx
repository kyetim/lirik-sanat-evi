...
export default function ChatWidget() {
  ...
  const buildInitialMsgs = (): Msg[] => [
    { id: 'i1', role: 'bot', text: ct.greeting1 },
    { id: 'i2', role: 'bot', text: ct.greeting2 },
  ]

  const [open, setOpen]       = useState(false)
  const [msgs, setMsgs]       = useState<Msg[]>(buildInitialMsgs)
  ...
  const send = async () => {
    ...
    try {
      ...
      const reply =
        data?.output ?? data?.text ?? data?.message ?? ct.connError
      setMsgs(m => [...m, { id: uid(), role: 'bot', text: reply }])
    } catch {
      setMsgs(m => [
        ...m,
        { id: uid(), role: 'bot', text: ct.connError },
      ])
    } finally {
      setLoading(false)
    }
  ...
  return (
    <>
            ...
            >
              <div className="flex items-end gap-2.5">
                <textarea
                  ...
                  rows={1}
                  disabled={loading}
                  className="flex-1 resize-none text-[0.875rem] leading-relaxed outline-none transition-colors duration-200 disabled:opacity-50 px-3 py-2.5"
                  style={{
              ...
              <p className="text-[10px] mt-2 tracking-wide" style={{ color: c.subtitle }}>
                {ct.sendHint}
              </p>
            </div>
          </motion.div>
      ...
      <motion.button
        onClick={() => setOpen(o => !o)}
        className="fixed z-[160] flex items-center justify-center bottom-[92px] right-6"
        style={{
          width: 52,
          height: 52,
          background: '#C9A84C',
        ...
        whileTap={{ scale: 0.93 }}
        aria-label={open ? ct.closeLabel : ct.openLabel}
      >
